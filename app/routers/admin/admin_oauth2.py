...
admin_oauth2_scheme = OAuth2PasswordBearer(tokenUrl="api/admin/signin") #path for authentication (login without the "/")
...
def create_access_token(data: dict):
    ...
    to_encode.update({"exp": expire}) # python dic syntax to add expiring time to the copied dict_data
...
def verify_access_token(token:str, credentials_exception):
    try:
        ...
        id: str = payload.get("admin_id") # user data is extracted from the decoded token
        if id is None:
...
def get_current_user(token: str = Depends (admin_oauth2_scheme), db: database.SessionLocal = Depends(database.get_db)):
