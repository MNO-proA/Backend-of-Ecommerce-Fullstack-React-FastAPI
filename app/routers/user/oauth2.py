...
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="api/signin") #path for authentication (login without the "/")
...
async def get_current_user(token: str = Depends (oauth2_scheme), db: db_setup.SessionLocal = Depends(db_setup.get_db)):
